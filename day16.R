...
# 지도 응용
df <- read.csv("data/전국전기차충전소표준데이터.csv", header=T, sep=",", encoding="UTF-8", fileEncoding = "euc-kr")       
str(df) 
head(df); View(df)
...
devtools::install_github("cardiomoon/kormaps2014") # https://rpubs.com/cardiomoon/222145
library(kormaps2014)
library(dplyr)
...
korpop1 <- rename(korpop1,
                  pop = 총인구_명,
                  name = 행정구역별_읍면동)
# korpop1$name <- iconv(korpop1$name, "UTF-8","CP949") 윈도우만..
